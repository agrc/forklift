...
def send_to_slack(url, messages):
    ...
    for message in messages:
        response = requests.post(url, data=message, headers={"Content-Type": "application/json"})
...
def _send_email_with_sendgrid(email_server, to, subject, body, attachments=[]):
    ...
    message = Mail(from_email=from_address, to_emails=to, subject=subject, html_content=body)
...
def _send_email_with_smtp(email_server, to, subject, body, attachments=[]):
    ...
    for path in attachments:
        ...
        attachment.add_header("Content-Disposition", f'attachment; filename="{path.name}.gz"')
