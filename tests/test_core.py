...
def test_update_no_existing_destination(test_gdb):
    ...
    assert arcpy.Exists(crate.destination) is True
...
def test_check_schema_ignore_length_for_all_except_text(test_gdb):
    ...
    result = core.check_schema(
        Crate(r"UPDATE_TESTS.DBO.Hello\UPDATE_TESTS.DBO.DNROilGasWells", UPDATE_TESTS_SDE, test_gdb, "DNROilGasWells")
    )
    assert result is True
...
def test_check_schema_no_objectid_in_source(test_gdb):
    ...
    result = core.check_schema(
        Crate("UPDATE_TESTS.dbo.NO_OBJECTID_TEST", UPDATE_TESTS_SDE, test_gdb, r"NO_OBJECTID_TEST")
    )
    assert result is True
...
def test_check_schema_match(test_gdb):
    ...
    assert core.check_schema(Crate("ZipCodes", test_gdb, test_gdb, "ZipCodes2")) is True
...
def test_create_destination_data_feature_class(test_gdb):
    ...
    assert arcpy.Exists(fc_crate.destination) is True
...
def test_create_destination_data_table(test_gdb):
    ...
    assert arcpy.Exists(tbl_crate.destination) is True
...
def test_check_counts(test_gdb):
    ...
    assert core._check_counts(crate, changes) is None
...
def test_schema_ignore_non_standard_shape_length_fields(test_gdb):
    result = core.check_schema(
        Crate("DirectionalSurveyHeaderSource", test_gdb, test_gdb, "DirectionalSurveyHeaderDestination")
    )
...
def test_is_nonhashable_field():
    ...
    assert core._is_nonhashable_field("SHAPE", describe_mock) is True
    assert core._is_nonhashable_field("OBJECTID_1", describe_mock) is True
    assert core._is_nonhashable_field("OBJECTID", describe_mock) is True
    assert core._is_nonhashable_field("Globalid", describe_mock) is False
    assert core._is_nonhashable_field("GoodField", describe_mock) is False
    assert core._is_nonhashable_field("FORKLIFT_HASH", describe_mock) is True
